...
Quarto is an open-source scientific and technical publishing system. [Quarto](https://quarto.org/docs/get-started/) version `{python} quarto_version` is used to compile the website.

### python and Pipenv

Python `{python} python_version()` is what we are currently using. There are several IDEs that you can use. We've used VSCode. <!--and?-->
...
Install pipenv if you don't already have it.  From the project top-level folder, run in a CLI `pipenv install`.  This may take some time, as it creates and populates an environment with all the required packages.
